...
class BERTDataset(Dataset):
    def __init__(self, data_dir, tokenizer, seq_len, language_type, mode="tgt+src"):
        
        self.vocab = tokenizer.vocab
        self.tokenizer = tokenizer
        self.seq_len = seq_len
        self.sample_counter = 0
        # MODE: 1. tgt + src_trans 2.tgt + src 3.tgt + src + src_trans
        processor = DataProcessor()
        self.examples = processor.get_sep_tgt_train_examples(data_dir, language_type)
        test_examples = processor.get_sep_tgt_test_examples(data_dir, language_type)
        self.examples.extend(test_examples)
        if mode == "tgt+src":
            src_examples = processor.get_src_train_examples(data_dir)
            limit_len = min(len(self.examples), len(src_examples))
            orig_domain_examples = random.sample(src_examples, limit_len)
            self.examples.extend(orig_domain_examples)
        elif mode == "tgt+src_trans":
            src_trans_examples = processor.get_src_trans_train_examples(data_dir, language_type)
            limit_len = min(len(self.examples), len(src_trans_examples)) 
            orig_domain_examples = random.sample(src_trans_examples, limit_len)
            self.examples.extend(orig_domain_examples)
        elif mode == "tgt+src+src_trans":
            src_examples = processor.get_src_train_examples(data_dir)
            src_trans_examples = processor.get_src_trans_train_examples(data_dir, language_type)
            limit_len = min(len(self.examples), len(src_trans_examples), len(src_examples))
            orig_examples = random.sample(src_examples, limit_len)
            orig_trans_examples = random.sample(src_trans_examples, limit_len)
            self.examples.extend(orig_examples)
            self.examples.extend(orig_trans_examples)
        else:
...
def main():
    ...
    parser.add_argument("--data_mode",
                        default=None,
                        type=str,
                        help="[tgt+src, tgt+src_trans, tgt+src+src_trans]")
    parser.add_argument("--lang_type",
                        default=None,
                        type=str,
                        required=True,
                        help="The input language type")
    parser.add_argument("--bert_tokenizer", default=None, type=str, required=True,
                        help = "Bert pre-trained tokenization model ")
    ...
    if args.do_train:
        train_dataset = BERTDataset(args.data_dir, tokenizer, seq_len=args.max_seq_length, language_type=args.lang_type, mode=args.data_mode)
