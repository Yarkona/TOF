...
def compute_f1(predictions, correct, idx2Label):
    ...
    return "Precision: " + str(prec) + ", Recall: " + str(rec) + ", F1: " + str(f1)
...
def main():
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        # test_examples = processor.get_sep_tgt_dev_examples(args.data_dir, args.lang_type)
        # test_examples = processor.get_sep_tgt_train_examples(args.data_dir, args.lang_type)
        ...
        # file_name = args.lang_type+".train.bio"
        file_name = os.path.join(args.output_dir, "results.txt")
        eval_file = open(file_name, "w")
        for input_ids, input_mask, segment_ids, label_ids, label_mask, guids in tqdm(test_dataloader, desc="Testing"):
            ...
            labels_true_lst += trues_lst
 
            for guid, trues, preds in zip(guids, trues_lst, preds_lst):
                test_output_dict[guid.item()] = (test_examples[guid].text, test_examples[guid].label, trues, preds, label_map)
                for w, label, true, pred in zip(test_examples[guid].text, test_examples[guid].label, trues, preds):
                    gtag = idx2label[true]
                    # assert label == gtag
                    ptag = idx2label[pred]
                    eval_file.write(" ".join([w, gtag, ptag])+"\n")
                eval_file.write("\n")
        ...
        result = {'test_loss': test_loss,
                  ...
                  'test_f1': compute_f1(labels_pred_lst, labels_true_lst, idx2label)}
