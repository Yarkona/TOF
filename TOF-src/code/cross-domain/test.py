...
class DataProcessor(object):
    ...
    def get_sep_scitech_train_examples(self, data_dir):
        """ See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, "sep_scitech_train.pkl")), "twitter_train")
...
#             tokens = tokens[:(max_seq_length - 2)]
        ...
        for label, target_index in zip(labels, orig_to_tok_map):
            label_ids[target_index] = label_map["O"]
...
def main():
    ...
    if args.do_test and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        if args.mode=="train":
            test_examples = processor.get_sep_scitech_train_examples(args.data_dir)
            file_name =  "sci.train.bio"
        ...
        for input_ids, input_mask, segment_ids, label_ids, label_mask, guids in tqdm(test_dataloader, desc="Testing"):
            ...
            for guid, trues, preds in zip(guids, trues_lst, preds_lst):
                ...
                for w, label, true, pred in zip(test_examples[guid].text, test_examples[guid].label, trues, preds):
                    gtag = idx2label[true]
                    # if label != gtag:
                    #     print(test_examples[guid].text, len(test_examples[guid].text))
                    #     print(test_examples[guid].label, len(test_examples[guid].label))
                    #     print([idx2label[t] for t in trues], len(trues))
                    #     break
                    ptag = idx2label[pred]
                    eval_file.write(" ".join([w, ptag])+"\n")
                eval_file.write("\n")
