...
class MRCNERDataLoader(object):
    ...
    def convert_examples_to_features(self, data_sign="train", num_data_processor=1, logger=None):
        ...
        if num_data_processor == 1:
            cache_path = os.path.join(self.save_cache_path, "mrc-ner.{}.cache.{}".format(data_sign, str(self.max_seq_len)))
        ...
        def export_features_to_cache_file(idx, sliced_features, num_data_processor):
            cache_path = os.path.join(self.save_cache_path, "mrc-ner.{}.cache.{}.{}-{}".format(data_sign, str(self.max_seq_len), str(num_data_processor), str(idx)))
        ...
        path_to_preprocessed_cache = os.path.join(self.save_cache_path, "mrc-ner.{}.cache.{}.{}-*".format(data_sign, str(self.max_seq_len), str(num_data_processor)))
        ...
        for idx in range(num_data_processor):
            features_lst.append(multi_process_for_data.apply_async(MRCNERDataLoader.read_features_from_cache_file, args=(idx, self.save_cache_path, data_sign, self.max_seq_len, num_data_processor, logger)))
