...
from mrc_data_processor import Conll03Processor, MSRAProcessor, Onto4ZhProcessor, Onto5EngProcessor, GeniaProcessor, ACE2004Processor, ACE2005Processor, ResumeZhProcessor
from bert_mrc import BertQueryNER
from bert_tokenizer import BertTokenizer4Tagger 
from mrc_ner_evaluate  import flat_ner_performance
from optimizers import build_fp32_optimizer
...
def args_parser():
    ...
    parser.add_argument("--data_mode", default="tgt", type=str, help="[tgt src src+trans tgt+trans tgt+src+trans]")
    parser.add_argument("--lang_type", default=None, type=str, help="[es, de, nl, no]")
    ...
    parser.add_argument("--output_dir", type=str, default="./output_mrc_ner")
...
def load_data(config):
    ...
    tokenizer = BertTokenizer4Tagger.from_pretrained(config.bert_tokenizer, do_lower_case=config.do_lower_case)
    ...
    if config.debug:
        ...
        train_dataloader = dataset_loaders.get_dataloader(data_sign="dev", num_data_processor=config.num_data_processor)
    else:
    ...
    print(f"Number of Training Data: {train_instances}")
    print(f"Train Epoch {config.num_train_epochs}; Total Train Steps: {num_train_steps}; Warmup Train Steps: {config.warmup_steps}")
    print(f"Per GPU Train Batch Size: {per_gpu_train_batch_size}")

    return train_dataloader, dev_dataloader, num_train_steps, label_list 
...
def train(model, optimizer, scheduler,  train_dataloader, dev_dataloader, config, \
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            if global_step % config.checkpoint == 0:
                ...
                if config.only_train:
                    ...
                    print(f"{tmp_dev_loss}, {tmp_dev_acc}, {tmp_dev_prec}, {tmp_dev_rec}, {tmp_dev_f1}")
                    ...
                    if tmp_dev_f1 > dev_best_f1 :
                        ...
                        if config.export_model:
                            ...
                            output_model_file = os.path.join(config.output_dir, WEIGHTS_NAME)
                            torch.save(model_to_save.state_dict(), output_model_file)
                            output_config_file = os.path.join(cofing.output_dir, CONFIG_NAME)
                            with open(output_config_file, "w") as f:
                        ...
                        if not config.only_eval_dev:
                            model, tmp_test_loss, tmp_test_acc, tmp_test_prec, tmp_test_rec, tmp_test_f1 = eval_checkpoint(model, config, device, n_gpu, label_list, eval_sign="test")
                            print("......"*10)
                            print("TEST: loss, acc, precision, recall, f1")
                            print(f"{tmp_test_loss}, {tmp_test_acc}, {tmp_test_prec}, {tmp_test_rec}, {tmp_test_f1}")
    ...
    print(f"{dev_best_loss}, {dev_best_acc}, {dev_best_precision}, {dev_best_recall}, {dev_best_f1}")
    if not config.only_eval_dev:
        print("scores on TEST when Best DEV:loss, acc, precision, recall, f1 ")
        print(f"{test_loss_when_dev_best}, {test_acc_when_dev_best}, {test_pre_when_dev_best}, {test_rec_when_dev_best}, {test_f1_when_dev_best}")
    else:
        print("Please Evaluate the saved CKPT on TEST SET using the Best dev model. ")
        print(f"Best Dev Model is saved : {best_dev_model_path}")
        print("Please run [evaluate_mrc_ner.py] to get the test performance. ")
    print("=&="*15)
...
def main():
    ...
    train_loader, dev_loader, num_train_steps, label_list = load_data(config)
    model, optimizer, scheduler, device, n_gpu = load_model(config, num_train_steps, label_list)
    train(model, optimizer, scheduler, train_loader, dev_loader, config, device, n_gpu, label_list)
