...
class DataProcessor(object):
    ...
    def get_src_train_examples(self, data_dir):
        ...
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, "en_conll_train.pkl")), "conll_train")

    def get_src_dev_examples(self, data_dir):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, "en_conll_test.pkl")), "conll_dev")

    def get_src_trans_train_examples(self, data_dir, language_type):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, language_type+"_conll_train.pkl")), "conll_train")

    def get_src_trans_dev_examples(self, data_dir, language_type):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, language_type+"_conll_test.pkl")), "conll_dev")

    def get_sep_tgt_train_examples(self, data_dir, language_type):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, language_type+"_sep_train.pkl")), language_type + "_train")

    def get_sep_tgt_test_examples(self, data_dir, language_type):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, language_type +"_sep_test.pkl")), language_type + "_test")
    
    def get_sep_tgt_dev_examples(self, data_dir, language_type):
        """See base class."""
        return self._create_examples(
            self._read_pkl(os.path.join(data_dir, language_type +"_sep_dev.pkl")), language_type + "_dev")
...
def main():
    ...
    if args.do_train:
        if args.supervised_training:
            train_examples = processor.get_sep_tgt_train_examples(args.data_dir, args.lang_type)
        else:
            if args.data_mode == "src":
                train_examples = processor.get_src_train_examples(args.data_dir)
            elif args.data_mode == "tgt":
                train_examples = processor.get_src_trans_train_examples(args.data_dir, args.lang_type)
            elif args.data_mode == "pseudo":
                train_examples = processor.get_sep_tgt_train_examples(args.data_dir, args.lang_type)
            elif args.data_mode == "tgt+src":
                train_examples = processor.get_src_train_examples(args.data_dir)
                trans_examples = processor.get_src_trans_train_examples(args.data_dir, args.lang_type)
                limit_len = min(len(train_examples), len(trans_examples))
                orig_domain_examples = random.sample(trans_examples, limit_len)
                train_examples.extend(orig_domain_examples)
            elif args.data_mode == "tgt+src+pseudo":
                train_examples = processor.get_src_train_examples(args.data_dir)
                tgt_train_examples = processor.get_sep_tgt_train_examples(args.data_dir, args.lang_type)
                trans_examples = processor.get_src_trans_train_examples(args.data_dir, args.lang_type)
                limit_len = min(len(train_examples), len(tgt_train_examples), len(trans_examples))
                add_examples_1 = random.sample(tgt_train_examples, limit_len)
                add_examples_2 = random.sample(trans_examples, limit_len)
                train_examples.extend(add_examples_1)
                train_examples.extend(add_examples_2)
            else:
                print("data model ERROR!!!!!!!!!!!!!!!!!!")
        total_examples = train_examples
        train_examples = random.sample(total_examples, round(len(total_examples) * args.data_ratio))
        print("data_size:{}, data_ratio:{}".format(len(train_examples), args.data_ratio))
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
                if args.data_mode == "src":
                    eval_examples = processor.get_src_dev_examples(args.data_dir)
                elif args.data_mode == "tgt":
                    eval_examples = processor.get_src_trans_dev_examples(args.data_dir, args.lang_type)
                elif args.data_mode == "pseudo":
                    eval_examples = processor.get_sep_tgt_dev_examples(args.data_dir, args.lang_type)
                elif args.data_mode == "tgt+src" or args.data_mode == "tgt+src+pseudo":
                    eval_examples = processor.get_src_dev_examples(args.data_dir)
                    trans_examples = processor.get_src_trans_dev_examples(args.data_dir, args.lang_type)
                    limit_len = min(len(eval_examples), len(trans_examples))
                    orig_domain_examples = random.sample(trans_examples, limit_len)
                    eval_examples.extend(orig_domain_examples)
                # elif args.data_mode == "tgt+src+pseudo":
                #     eval_examples = processor.get_src_dev_examples(args.data_dir)
                #     trans_examples = processor.get_src_trans_dev_examples(args.data_dir, args.lang_type)
                #     tgt_eval_examples = processor.get_tgt_dev_examples(args.data_dir)
                #     limit_len = min(len(eval_examples), len(trans_examples), len(tgt_eval_examples))
                #     add_examples_1 = random.sample(trans_examples, limit_len)
                #     add_examples_2 = random.sample(tgt_eval_examples, limit_len)
                #     eval_examples.extend(add_examples_1)
                #     eval_examples.extend(add_examples_2)
                ...
                if f1 > best_result["f1"]:
                    ...
                    test_examples = processor.get_sep_tgt_test_examples(args.data_dir, args.lang_type)
