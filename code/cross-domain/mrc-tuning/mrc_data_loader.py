...
class MRCNERDataLoader(object):
    ...
    def convert_examples_to_features(self, data_sign="train", num_data_processor=1, logger=None):
        ...
        if data_sign == "train":
            total_examples = self.data_processor.get_train_examples(self.data_dir)
            examples = random.sample(total_examples, round(len(total_examples) * self.data_ratio))
            print("data_ratio:{}, data_size:{}".format(self.data_ratio, len(examples)))
        ...
        if num_data_processor == 1:
            cache_path = os.path.join(self.data_dir, "mrc-ner.{}.{}.cache.{}".format(self.data_ratio, data_sign, str(self.max_seq_len)))
        ...
        def export_features_to_cache_file(idx, sliced_features, num_data_processor):
            cache_path = os.path.join(self.data_dir, "mrc-ner.{}.{}.cache.{}.{}-{}".format(self.data_ratio, data_sign, str(self.max_seq_len), str(num_data_processor), str(idx)))
        ...
        path_to_preprocessed_cache = os.path.join(self.data_dir, "mrc-ner.{}.{}.cache.{}.{}-*".format(self.data_ratio, data_sign, str(self.max_seq_len), str(num_data_processor)))
        ...
        if len(collection_of_preprocessed_cache) == num_data_processor:
            ...
            print(f"%%%% %%%% Load Saved Cache files in {self.data_dir} %%% %%% ")
        ...
        for idx in range(num_data_processor):
            features_lst.append(multi_process_for_data.apply_async(MRCNERDataLoader.read_features_from_cache_file, args=(idx, self.data_ratio, self.data_dir, data_sign, self.max_seq_len, num_data_processor, logger)))
