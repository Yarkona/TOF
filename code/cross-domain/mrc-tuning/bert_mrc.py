...
from classifier import MultiNonLinearClassifier, SingleNonLinearClassifier
import pytorch_pretrained_bert 
from pytorch_pretrained_bert.modeling import BertPreTrainedModel, BertModel, BertConfig, WEIGHTS_NAME


class BertQueryNER(nn.Module):
    def __init__(self, config, train_steps=12000000):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        if start_positions is not None and end_positions is not None:
            ...
            start_loss = torch.sum(start_loss * token_type_ids.float().view(-1))
            start_loss = start_loss / valid_num.float()
            end_loss = loss_fct(end_logits.view(-1, 2), end_positions.view(-1))
            end_loss = torch.sum(end_loss * token_type_ids.float().view(-1))
            ...
            span_loss = torch.sum(span_loss.view(-1) * span_label_mask.float().view(-1))
